...
// ======= T_Complex unit tests =======

TEST_F(NeblinaCoreFixture, vector_new_complex) {

    int n = 4;

    vector_t * a = m.bridges[idx].vector_new(n, T_COMPLEX, 0 );

    ASSERT_EQ(4, a->len);
    ASSERT_EQ(LOCHOS, a->location);
    ASSERT_EQ(T_COMPLEX, a->type);
    ASSERT_TRUE(&(a->value.f) != nullptr);
    ASSERT_EQ(NULL, a->extra);
...
TEST_F(NeblinaCoreFixture, vector_new_empty_complex) {
...
TEST_F(NeblinaCoreFixture, vector_vecreqdev_complex) {
...
TEST_F(NeblinaCoreFixture, matrix_new_complex) {
...
TEST_F(NeblinaCoreFixture, matrix_matreqdev_complex) {
...
TEST_F(NeblinaCoreFixture, convertToObject_withNULL_complex) {
